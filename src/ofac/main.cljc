(ns ofac.main
  "Kotodama WASM entrypoint for the OFAC clean-room actor (L4) — Clojure port.
  ...
  facts (`ofac.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"ofac.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ofac-compat" :tier tier :entities entities} 200])
